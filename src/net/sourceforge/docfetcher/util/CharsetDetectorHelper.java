...
public final class CharsetDetectorHelper {
	...
	public static String toString(@NotNull byte[] bytes)
		...
		try {
			...
			charsetName = charsetDetector.getDetectedCharset();
		}
		catch (ArrayIndexOutOfBoundsException e) {
			/*
			 * Bug #2235: ArrayIndexOutOfBoundsException in universalchardet's
			 * SequenceModel.getPrecedence() when processing certain files with
			 * unusual byte sequences. Fall back to ISO_8859_1 encoding.
			 */
			charsetName = null;
		}
		finally {
		...
		try {
			...
			/*
			 * Bug #2422: NegativeArraySizeException when processing corrupted
			 * or malformed files where charset decoding results in invalid
			 * size calculations.
			 */
			throw new IOException("File too large or corrupted for charset decoding", e);
		}
	}
