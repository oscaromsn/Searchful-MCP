	...
	public ErrorTable(@NotNull Composite parent) {
		...
		tv.addColumn(new Column<IndexingError>(Msg.path.get()) {
			...
			protected int compare(IndexingError e1, IndexingError e2) {
				...
				File f2 = null;
				try {
					f1 = e1.getTreeNode().getPath().getCanonicalFile();
				} catch (NullPointerException e) {
					/* Similar to bug #1128: file path may be null if deleted */
				}
				try {
					f2 = e2.getTreeNode().getPath().getCanonicalFile();
				} catch (NullPointerException e) {
					/* Similar to bug #1128: file path may be null if deleted */
				}

				// Handle null cases: nulls sort to the end
				if (f1 == null && f2 == null)
					return 0;
				if (f1 == null)
					return 1;
	...
	private void copySelectedErrorsToClipboard() {
		...
		for (IndexingError error : sel) {
			...
			try {
				...
				/*
				 * Similar to bug #1128: The file path may be null if the
				 * file was deleted or is no longer accessible when copying
				 * error information to the clipboard.
				 */
			}
			if (file == null)
				continue;
